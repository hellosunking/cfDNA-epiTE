...
a = data.frame( predict$Type, predict$pred, sample$stage )
colnames(a) = c("Type", "pred", "stage")
...
p=vector()
for (i in 1:5 ){
	c <- subset(a, stage=="0.Healthy" | stage==b[i])
	...
	results[[i]] <- list(sensitivity = sensitivity, specificity = specificity,
						 ci_lower = ci$lower, ci_upper = ci$upper, num_positive_detected = num_positive_detected)

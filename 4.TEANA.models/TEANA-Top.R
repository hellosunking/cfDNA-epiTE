	...
	for (rep in c(1:10)) {
		...
		foreach(n = 1:10, .packages = c("caret", "pROC"), .combine = 'c', .verbose = F ) %dopar% {
			...
			ctrl <- trainControl(method="repeatedcv",number=10,repeats=10,savePredictions=TRUE,
								 classProbs=TRUE,search="random",summaryFunction=twoClassSummary,verboseIter=F)
			...
			param_grid <-expand.grid(n.trees= c(50,100,150,200,300),
						        interaction.depth = c(1,3,5), shrinkage=c(0.01,0.05,0.1,0.2,0.3),
			...
			model <- train( Type ~ ., data=train, method="gbm",
						   trControl=trainControl(method = "none", savePredictions = TRUE,
									classProbs = TRUE, summaryFunction = twoClassSummary, verboseIter = F),
							tuneGrid=param_grid, metric="ROC" )
			...
			write.table(test_pred.t,file = paste(b[i],"/test_pred",n,"_rep",rep,".txt",sep=""),

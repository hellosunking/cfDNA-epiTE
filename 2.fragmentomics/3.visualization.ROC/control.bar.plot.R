...
fct <- reorder(df$variable, -df$value) %>%
  fct_relevel("Genomewide", after = 0)
colour=rainbow(16)                   
df$G<-rep(c("SINE","LINE","SINE","LINE","LTR","DNA","LTR","LTR","DNA","LINE","DNA","DNA","LTR","DNA","LINE","LTR","Genomewide"), each = n)
ggplot(df,aes(x=fct,y=value,fill=G))+ 
  ...
  labs(x=NULL,y="size",title = "your title")+ 
  ...
  labs(fill = "subtype")+ 
  scale_fill_manual(values = c("#f8cb7f","grey","#63b2ee","#f89588","#7cd6cf"))+
  stat_compare_means(label = "p.signif", method = "t.test",paired = T,label.y = 0.95,
